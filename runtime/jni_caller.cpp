...
bool is_caller_class(JNIEnv* env, jobject obj)
{
    if(!callercls)
    {
        callercls = env->FindClass("metaffi/Caller");
...
{
    // Find the Caller class
    jclass callerClass = env->FindClass("metaffi/Caller");
    check_and_throw_jvm_exception(env, true);

    // Find the createCaller method in Caller class
    jmethodID createCallerMethod = env->GetStaticMethodID(callerClass, "createCaller", "(J[J[J)Lmetaffi/Caller;");
    check_and_throw_jvm_exception(env, true);
