...
func TestJavaHost(t *testing.T) {
	...
	cmd := exec.Command("javac", "-cp", fmt.Sprintf(".%vHostTest_MetaFFIHost.jar%v"+os.Getenv("METAFFI_HOME")+"/xllr.openjdk.bridge.jar", sep, sep), "HostTest.java")
	wd, _ := os.Getwd()
	cmd.Dir = wd
	...
	cmd = exec.Command("java", "-cp", fmt.Sprintf(".%vHostTest_MetaFFIHost.jar%v"+os.Getenv("METAFFI_HOME")+"/xllr.openjdk.bridge.jar", sep, sep), "HostTest")
	cmd.Dir = wd
	cmd.Stdout = os.Stdout
	cmd.Stderr = os.Stderr
	fmt.Printf("%v\n", strings.Join(cmd.Args, " "))
