	...
	public static Caller createCaller(long xcallAndContext, long[] parametersTypesArray, long[] retvalsTypesArray)
	{
		...
		Function<Object, Object[]> f = (Object parametersArray) ->
		{
			...
			if(paramsCount > 0 || retvalsCount > 0)
				xcall_params = MetaFFIBridge.alloc_cdts(paramsCount, retvalsCount);
			...
			if(paramsCount > 0)
				parametersCDTS = MetaFFIBridge.get_pcdt(xcall_params, (byte)0);

			if(retvalsCount > 0)
				returnValuesCDTS = MetaFFIBridge.get_pcdt(xcall_params, (byte)1);

			// fill CDTS
			if(paramsCount > 0)
				MetaFFIBridge.java_to_cdts(parametersCDTS, (Object[])parametersArray, parametersTypesArray);

			if(paramsCount > 0 && retvalsCount > 0)
				MetaFFIBridge.xcall_params_ret(xcallAndContext, xcall_params);
			if(paramsCount > 0 && retvalsCount == 0)
				MetaFFIBridge.xcall_params_no_ret(xcallAndContext, xcall_params);
			if(paramsCount == 0 && retvalsCount > 0)
				MetaFFIBridge.xcall_no_params_ret(xcallAndContext, xcall_params);
			if(paramsCount == 0 && retvalsCount == 0)
				MetaFFIBridge.xcall_no_params_no_ret(xcallAndContext);

			// fill result
			if(retvalsCount == 0)
				return null;

			Object[] res = MetaFFIBridge.cdts_to_java(returnValuesCDTS, retvalsCount);
			return res;
		};
